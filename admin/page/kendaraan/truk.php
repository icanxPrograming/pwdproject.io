    ...
    <table class="table table-hover table-bordered table-sm">
      <thead class="thead-light">
        <tr>
          ...
          <th scope="col" style="width: 150px;">Nama Truk</th>
          ...
          <th scope="col" style="width: 80px;">Kapasitas</th>
      ...
      <tbody>
        <?php if (empty($truck)): ?>
          <tr>
            <td colspan="11" class="text-center text-muted">Belum ada data truk.</td>
          </tr>
        <?php else: ?>
          <?php $nomor = 1; ?>
          <?php foreach ($truck as $row): ?>
            <tr>
              ...
              <td><?= htmlspecialchars($row['jenis_truk']) ?></td>
              <td><?= htmlspecialchars($row['tahun']) ?></td>
              <td><?= htmlspecialchars($row['bahan_bakar'] ?? '-') ?></td>
              <td><?= htmlspecialchars($row['transmisi'] ?? '-') ?></td>
              <td><?= htmlspecialchars($row['kapasitas_muatan'] ?? '-') ?></td>
              ...
              <td class="text-center">
                ...
                <a href="/PWD-Project-Mandiri/admin/page/kendaraan/hapus-kendaraan.php?type=truk&id=<?= $row['id_truk'] ?>"
                  onclick="return confirm('Yakin ingin menghapus <?= addslashes(htmlspecialchars($row['nama_truk'])) ?>?');"
